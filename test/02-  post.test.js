...
describe('Test Unit for all POST routes on /api/v1', function() {
  ...
  describe('POST /api/v1/course', function() {
    ...
    before(done => {
      ...
      it("Do save a new course on database if teachers field recieve less than 2 valid teachers", function() {
        return request(app)
          .post('/api/v1/course')
          .send({ name: "New Test Course", period: 2, city: "Ipatinga", teacher: [1, 2] })
          .then(function(res) { 
            assert.equal(res.status, 201);
          });
      });
      ...
      it("Don't save a new course on database if teachers field recieve less than 2 valid teachers", function() {
        return request(app)
          .post('/api/v1/course')
          .send({ name: "New Test Course", period: 2, city: "Ipatinga", teacher: [1, 'invalid'] })
          .then(function(res) { 
            assert.equal(res.status, 401);
          });
      });
  ...
  describe('POST /api/v1/student', function() {
    ...
    it("Do save a new student on database if age >= 17", function() {
      return request(app)
        .post('/api/v1/student')
        .send({ name: "John", lastname: "Doe", age: 17, course: 2})
        .then(function(res) { 
          assert.equal(res.status, 201);
        });
    });
    ...
    it("Don't save a new student on database if its age is less than 17", function() {
      return request(app)
        .post('/api/v1/student')
        .send({ name: "John", lastname: "Doe", age: 16, course: 2})
        .then(function(res) { 
          assert.equal(res.status, 401);
        });
    });
    ...
    it("Do save a new student if it has a valid course", function() {
      return request(app)
        .post('/api/v1/student')
        .send({ name: "John", lastname: "Doe", age: 21, course: 2})
        .then(function(res) { 
          assert.equal(res.status, 201);
        });
    });
